...
def plot_xfit(fit_info, save_name, location='./figures/'):
    ...
    if 'seq2seq_loss_list' in fit_info.dict and 'seq2seq_vloss_list' in fit_info.dict:
        seq2seq_loss,seq2seq_vloss = np.array(fit_info.seq2seq_loss_list), np.array(fit_info.seq2seq_vloss_list)
        agent_loss,agent_vloss = np.array(fit_info.agent_loss_list), np.array(fit_info.agent_vloss_list)
        f = plt.figure()
        plt.plot(x, seq2seq_loss[:num_samples], label='Training_seq2seq',ls = ':')
        plt.plot(x, seq2seq_vloss[:num_samples], label='Validation_seq2seq',ls = ':')
        plt.plot(x, agent_loss[:num_samples], label='Training_agent')
        plt.plot(x, agent_vloss[:num_samples], label='Validation_agent')
        plt.legend()
        f.savefig(os.path.join(location, save_name + '_AgentSeq2seq' +'.xfit.png'))
        np.save(os.path.join(location, save_name) + '_Seq2seq'+ '.loss', (seq2seq_loss,seq2seq_vloss))
        np.save(os.path.join(location, save_name) + '_Agent'+ '.loss', (agent_loss,agent_vloss))
        plt.close()
