...
def mae(target, pred, ith=None):
    return np.mean(np.fabs(pred - target)).item()


def mse(target, pred, ith=None):
...
def mase(target, pred, avgNaiveError, ith = None):
    '''
    ...
    def h_mase(target,pred, avgNaiveError, ith):
        '''
        Computing the mase at i_th of the prediction horizon.
        '''
        avgNaiveError = avgNaiveError[:pred.shape[0]]
        t = target[:,ith-1]/avgNaiveError
        p = pred[:, ith-1]/avgNaiveError
    ...
    if ith is None:
        ...
        _h_mase  = np.zeros(H)
        for h in range(H):
            _h_mase[h] = h_mase(target,pred,avgNaiveError,h+1)
        output = _h_mase.mean().item()
    else:
        output = h_mase(target,pred,avgNaiveError,ith)
