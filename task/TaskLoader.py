...
from task.dataset import mlp_dataset, dnn_dataset,rnn_dataset,rnn_patch_dataset,Dataset_normal,Dataset_patch,Dataset_nonpatch
from task.dataset import difference, create_dataset,create_2d_dataset
...
class TaskDataset(Opt):
    ...
    def loader_dataset(self,raw_ts):


        # num_train = int(len(raw_ts) * 0.2)
        # num_test = int(len(raw_ts) * 0.7)
 

        num_train = int(len(raw_ts) * 0.7)
        num_test = int(len(raw_ts) * 0.2)
        num_vali = len(raw_ts) - num_train - num_test
        border_start = [0, num_train - self.info.steps, len(raw_ts) - num_test - self.info.steps]
        border_end = [num_train, num_train + num_vali, len(raw_ts)]

        train_idx = [border_start[0], border_end[0]]
        valid_idx = [border_start[1], border_end[1]]
        test_idx = [border_start[2], border_end[2]]


        if self.info.feature == 'M' or self.info.feature == 'MS':
            df_data = raw_ts
        elif self.info.feature == 'S':
            df_data = raw_ts[:,-1]
        
        sub = Opt()
        sub.scaler = StandardScaler()
        sub.scaler.fit(df_data)
        dataset = sub.scaler.transform(df_data)
        # if df_data.shape[0] > self.info.steps:
        #     ones = np.ones((df_data.shape[0], self.info.steps))
        #     data_cat = np.concatenate((ones, df_data), axis=1)
        #     sub.scaler.fit(data_cat)
        #     dataset = sub.scaler.transform(data_cat)





        train_data, valid_data, test_data = dataset[train_idx[0]:train_idx[1]], dataset[valid_idx[0]:valid_idx[1]], dataset[test_idx[0]:test_idx[1]]

        if self.info.if_patch:
            Dataset_form = Dataset_patch
        else:
            Dataset_form = Dataset_nonpatch
        
        train_dataset = Dataset_form(train_data,info=self.info)
        valid_dataset = Dataset_form(valid_data,info=self.info)
        test_dataset = Dataset_form(test_data,info=self.info)
        train_loader = DataLoader(train_dataset, batch_size=self.info.batch_size,drop_last=True, shuffle=False,pin_memory=self.args.cuda)
        valid_loader = DataLoader(valid_dataset, batch_size=self.info.batch_size,drop_last=True, shuffle=False,pin_memory=self.args.cuda)
        test_loader = DataLoader(test_dataset, batch_size=self.info.batch_size, drop_last=True,shuffle=False,pin_memory=self.args.cuda) 
    ...
    def pack_dataset(self, raw_ts):
        ...
        if num_variate > 1:
            ...
            _dataset,train_idx, valid_idx, test_idx = self.data_split(raw_ts)#分割数据
            #for testing
            sub.orig = _dataset
    ...
    def data_split(self,raw_ts):
        # 切割数据,划分训练集、验证集、测试集,首先分割训练测试，然后分割训练集和验证集
        if self.info.if_patch:
            self.history_len = self.info.steps + self.info.H - 1
        else:
            self.history_len = self.info.steps
        
        dataset = create_dataset(raw_ts,self.history_len,self.info.H)

        if len(raw_ts.shape) > 1:
            dataset = create_2d_dataset(raw_ts,self.history_len,self.info.diff)
        else:
            dataset = create_dataset(raw_ts,self.history_len )
    ...
    def data_loader(self, train_data, valid_data, test_data,otherVariate_list = None):
        ...
        if arch == 'deepar':
            ...
            elif arch == 'seq2seq' and self.info.patch_size == 0:
                dataform = rnn_dataset
            elif arch == 'seq2seq' and self.info.patch_size > 0:
                dataform = rnn_patch_dataset ##patch折叠数据
            else:
                assert False
                
            if otherVariate_list is not None and arch == 'seq2seq':
                if self.info.patch_size == 0:
                    ...
                    train_set, _, _ = dataform(train_data, self.info.H, self.info.steps,self.info.patch_size,otherVariate_list[0])
                    valid_set, _, _ = dataform(valid_data, self.info.H, self.info.steps,self.info.patch_size,otherVariate_list[1])
                    test_set, _, _ = dataform(
                        test_data, self.info.H, self.info.steps,self.info.patch_size,otherVariate_list[2]) 
                ...
                if self.info.patch_size == 0:
                    ...
                    train_set, _, _ = dataform(train_data, self.info.H, self.info.steps,self.info.patch_size)
                    valid_set, _, _ = dataform(valid_data, self.info.H, self.info.steps,self.info.patch_size)
                    test_set, _, _ = dataform(
                        test_data, self.info.H, self.info.steps,self.info.patch_size) 
            ...
            train_loader = DataLoader(train_set, batch_size= train_batch_size , pin_memory=self.args.cuda, shuffle=False, sampler=SequentialSampler(train_set),drop_last=True)
            
            valid_loader = DataLoader(valid_set, batch_size=valid_batch_size, pin_memory=self.args.cuda, shuffle=False, sampler=SequentialSampler(valid_set),drop_last=True)

            test_loader = DataLoader(test_set, batch_size=test_batch_size, pin_memory=self.args.cuda, shuffle=False, sampler=SequentialSampler(test_set),drop_last=True)
...
def data_loader(args, train_data, valid_data, test_data):
    ...
    if arch == 'deepar':
        ...
        train_loader = DataLoader(train_set, batch_size= train_batch_size , pin_memory=args.cuda, shuffle=False, sampler=SequentialSampler(train_set),drop_last=True)
        
        valid_loader = DataLoader(valid_set, batch_size=valid_batch_size, pin_memory=args.cuda, shuffle=False, sampler=SequentialSampler(valid_set),drop_last=True)
